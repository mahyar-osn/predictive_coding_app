"""
Copied from the original source code at https://github.com/C16Mftang/temporal-predictive-coding
"""
...
class TPC:
    def __init__(self, input_array: np.ndarray, A: np.ndarray, C: np.ndarray, dt: float = 0.5, k1: float = 0.001,
        ...
        if activation == 'linear':
            from utils.activation import linear as f
            from utils.activation import linear_deriv as df
            self.f = f
            self.df = df
        elif activation == 'nonlinear':
            from utils.activation import tanh as f
            from utils.activation import tanh_deriv as df
            self.f = f
            self.df = df
        else:
